...
export function initConfig(context: InvocationContext): Config {
  const config = {
    ...
    connector: {
      ...
      template: template,
      jsonApiUrl: process.env.JSON_API_URL || 'https://www.microsoft.com/releasecommunications/api/v1/m365',
    },
  };
...
export function validateConfig(config: Config): void {
  ...
  if (!config.connector.jsonApiUrl) {
    throw new Error("Invalid configuration: Missing JSON API URL");
  }
